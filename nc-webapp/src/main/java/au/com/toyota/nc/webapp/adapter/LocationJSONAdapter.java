 ...
 * Time: 17:47:20
 */
public class LocationJSONAdapter implements JSONAdapter{
    ...
    public LocationJSONAdapter(Location location) {
        this.location = location;
    }

    public JSONObject toJSONObject(){
        JSONObject result = new JSONObject();
        result.put(PROPERTY_LOCATION_ID, location.getLocationId());
        result.put(PROPERTY_LOCATION_NAME, location.getLocationName());
        result.put(PROPERTY_LOCATION_TYPE, location.getType());
        if (location.getContact() != null && location.getContact().getStreetAddress()!=null) {
            if (location.getContact().getStreetAddress() != null) {
        ...
        if (location.getDealer().getGeneralInfo().getPma() != null) {
